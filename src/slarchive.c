...
static void dr_msg_cb (rd_kafka_t *rk, const rd_kafka_message_t *rkmessage,
                       void *opaque);
static char *brokers = NULL;     /* Kafka brokers */
static char *topic   = NULL;     /* Kafka topic   */
static int npartitions = 0;      /* the number of partitions of the topic */
  ...
  if (brokers != NULL && topic != NULL) 
    {
      ...
      if (rd_kafka_conf_set(conf, "bootstrap.servers", brokers, err_str,
      ...
      rd_kafka_conf_set(conf, "queue.buffering.max.ms", "1", NULL, 0);
      rk = rd_kafka_new(RD_KAFKA_PRODUCER, conf, err_str, sizeof(err_str));
      if (rk == NULL) 
        {
          fprintf(stderr, "failed to create the Kafka producer: %s\n", err_str);
          exit(1);
        }
      if ((rkt = rd_kafka_topic_new(rk, topic, NULL)) == NULL) 
        {
          fprintf(stderr, "failed to create the topic %s: %s\n", topic,
                           rd_kafka_err2str(rd_kafka_last_error()));
          rd_kafka_destroy(rk);
          exit(1);
        }
      if (rd_kafka_metadata(rk, 0, rkt, &metadata, 5000) != 
      ...
      npartitions = metadata->topics[0].partition_cnt;
    }
...
packet_handler (char *msrecord, int packet_type, int seqnum, int packet_size)
{
  ...
  if ( npartitions > 0 && archflag != 0) {
    int i, n;
    char key[16];
    key[0] = msr->fsdh.network[0];
    key[1] = msr->fsdh.network[1];
    for (i = 0; i < 5; i++) {
      key[2+i] = msr->fsdh.station[i];
      if (key[2+i] == ' ')
        break;
    }
    key[2+i] = '\0';
    i = hash(key) % npartitions;
    n = rd_kafka_produce(rkt, i, RD_KAFKA_MSG_F_COPY, msr->msrecord, SLRECSIZE, 
                         NULL, 0, NULL);
    if (n == -1) {
      fprintf(stderr, "failed to publish %s.%s.%s.%s to topic %s: %s\n",
        msr->fsdh.network, msr->fsdh.station, msr->fsdh.location,
        msr->fsdh.channel, rd_kafka_topic_name(rkt), 
        rd_kafka_err2str(rd_kafka_last_error()));
    }
    rd_kafka_poll(rk, 0);
  }
...
parameter_proc (int argcount, char **argvec)
{
  ...
  for (optind = 1; optind < argcount; optind++)
      ...
      else if (strcmp (argvec[optind], "-tw") == 0)
	{
      ...
      else if (strcmp (argvec[optind], "-bs") == 0)
        {
          brokers = getoptval(argcount, argvec, optind++);
        }
      else if (strcmp (argvec[optind], "-to") == 0)
        {
          topic = getoptval(argcount, argvec, optind++);
        }
...
usage (int level)
{
  ...
  fprintf (stderr,
	   ...
	   "\n"
           ...
           " -bs servers     Define a list of Kafka bootstrap servers\n"
           "                 'servers' = 'host1:9092[,host2:9092,...]'\n"
           " -to topic       Write records into the topic\n"
           "\n"
